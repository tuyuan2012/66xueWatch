...
//  66xueWatchDemo WatchKit Extension
//
...
- (IBAction)pushToCircleListInterface {
    ...
    [WKInterfaceController openParentApplication:userInfo reply:^(NSDictionary *replyInfo, NSError *error) {
        ...
        if ([[replyInfo objectForKey:@"code"]isEqualToString:@"0000"]) {
            ...
            //跳转至微博列表 并将微博模型数据传入
            [self pushControllerWithName:@"CircleListInterface" context:newModels];
        }
    }];
    } else {
        //数据库有本地数据，直接跳转
        [self pushControllerWithName:@"CircleListInterface" context:models];
    }
}
